...
{
    public class ForceClientSalesforceProvider<T> : SalesforceProviderBase<T> 
        where T : class
    {
        public ForceClientSalesforceProvider(
            ForceClient forceClient, 
            ISalesforceNamingConvention objectNameProvider,
            SelectTypeEnum selectType = SelectTypeEnum.SelectIdAndUseAttachModel) 
            : base(objectNameProvider, selectType)
        {
            ForceClient = forceClient;
        }
